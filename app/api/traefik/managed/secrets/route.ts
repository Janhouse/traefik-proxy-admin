...
import { getManagedSecrets, updateManagedSecrets } from "@/lib/app-config";
 ...
 * resulting NAMES — values are write-only and never echoed back.
 */
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const result = applySecretEdits(await getManagedSecrets(), edits);
    if (!result.ok) {
      return NextResponse.json({ errors: result.errors }, { status: 400 });
    }
    await updateManagedSecrets(result.value);
    return NextResponse.json({ secretNames: Object.keys(result.value).sort() });
  } catch (error) {
