  ...
  return (
    <Card className="bg-card border overflow-hidden">
      <CardContent className="p-0">
        <Table>
          <TableHeader>
            <TableRow className="border-b border-[var(--border-soft)]">
          ...
          <TableBody>
            {domains.map((domain) => (
              <TableRow key={domain.id} className="border-b border-[var(--border-soft)] last:border-0">
                <TableCell>
                  {domain.isDefault && (
                    <div title="Default Domain">
                      <Star className="h-4 w-4 text-[var(--warn)] fill-current" />
                    </div>
                  )}
                </TableCell>
                ...
                <TableCell>
                  <code className="mono text-sm bg-[var(--surface-2)] px-2 py-1 rounded border border-[var(--border-soft)]">
                    {domain.domain}
                  </code>
                </TableCell>
                <TableCell>
                  <Badge
                    variant={domain.useWildcardCert ? "default" : "secondary"}
                    className="gap-1"
                  >
